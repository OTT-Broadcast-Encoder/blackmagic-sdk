...
void	print_attributes (IDeckLink* deckLink, bool showConnectorAttributes)
{
	...
	result = deckLinkAttributes->GetString(BMDDeckLinkEthernetMACAddress, &stringValue);
	if (result == S_OK)
	{
		auto stdString = DlToStdString(stringValue);
		DeleteString(stringValue);
		printf(" %-40s %s\n", "MAC Address:", stdString.c_str());
	}
	else if (result == E_NOTIMPL)
	{
		// Not present on product
	}
	else
	{
		fprintf(stderr, "Could not query BMDDeckLinkEthernetMACAddress. result = %08x\n", result);
	}
			...
			for (const auto& conversion: gOutputConversions)
			{
				...
				if (connection.first == bmdVideoConnectionSDI || connection.first == bmdVideoConnectionOpticalSDI)
				{
					for (std::list<std::pair<BMDSupportedVideoModeFlags, std::string>>::const_reverse_iterator links = gSDILinks.rbegin(); links != gSDILinks.rend(); links++)
					{
						...
						print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(links->first | bmdSupportedVideoModePsF), header);
						if (multilinkSupported)
						{
							...
							print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(links->first | bmdSupportedVideoModePsF | bmdSupportedVideoModeDualStream3D), header);
						}
					}
				}
				else
				{
					...
					print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, bmdSupportedVideoModePsF, header);
				...
				{
					snprintf(header, kMaxHeaderLength, "\n%s keyer output:", connection.second.c_str());
	...
	for (auto connection: gConnections)
	{
		if (connection.first == bmdVideoConnectionUnspecified || (ports & connection.first))
		{
			...
			for (const auto& conversion: gInputConversions)
			{
				...
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, bmdSupportedVideoModePsF, "PsF", header);
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, bmdSupportedVideoModeDualStream3D, "3D", header);
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(bmdSupportedVideoModePsF | bmdSupportedVideoModeDualStream3D), "PsF3D", header);
			}
		}
	}
